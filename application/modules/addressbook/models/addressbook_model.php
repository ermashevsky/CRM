...
//                $tmp->organization_id = $row->organization_id;
    ...
    function getAllTableData() {
        ...
        $this->db->select("*, organization.initiator, organization.id, organization.email, organization.address,COUNT( contacts.id ) AS counter_members", FALSE);
        $this->db->from('organization');
        ...
        if (0 < $res->num_rows) {
            foreach ($res->result() as $row) {
                ...
                $tmp = new Addressbook_model();
                if (($row->initiator === $data['user']->first_name . ' ' . $data['user']->last_name) || is_null($row->initiator)) {
                    $tmp->id = $row->id;
                    $tmp->organization_name = $row->organization_name;
    ...
    function getAllContactsTableData() {
        ...
        $this->db->select("*, contacts.initiator, contacts.id, contacts.email, contacts.address, contacts.comment, organization.organization_name", FALSE);
        $this->db->from('contacts');
