    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function saveNewContact() {
                ...
                function (data) {
                    window.location.href = "/addressbook#contacts_list";
                });
            }
...
//                console.info(call_type);
            ...
            function completeRec() {
                ...
                $.post('<?php echo site_url('/records/getExecutionEndRec'); ?>',
                        function (data) {
                            ...
                            var t = $('table#allContactsTable').DataTable();
                            var n = 1;
                            $.each(data, function (i, val) {

                                if (data[i].task_name !== "") {
                                    t.dataTable().fnAddData([n++, '<a href="records/viewTask/' + data[i].id + '">' + data[i].task_name + '</a>', data[i].initiator, data[i].assigned, new Date(data[i].create_date).format('dd.mm.yyyy HH:mm:ss'), new Date(data[i].end_date).format('dd.mm.yyyy HH:mm:ss'), '<div class="btn-group"><button type="button" class="btn btn-small btn-info" title="Выполнено" onclick="doneRecord(' + data[i].id + '); return false;"><i class="icon-check"> </i></button><button type="button" class="btn btn-small btn-success" title="Редактировать"  onclick="redirect2EditRecord(' + data[i].id + '); return false;"><i class="icon-edit"> </i></button><button type="button" class="btn btn-small btn-danger" title="Удалить" onclick="delRecord(' + data[i].id + '); return false;"><i class="icon-trash"> </i></button></div>']);
                                } else {
                                    t.dataTable().fnAddData([n++, '<a href="records/viewTask/' + data[i].id + '">' + data[i].task_description + '</a>', data[i].initiator, data[i].assigned, new Date(data[i].create_date).format('dd.mm.yyyy HH:mm:ss'), new Date(data[i].end_date).format('dd.mm.yyyy HH:mm:ss'), '<div class="btn-group"><button type="button" class="btn btn-small btn-info" title="Выполнено" onclick="doneRecord(' + data[i].id + '); return false;"><i class="icon-check"> </i></button><button type="button" class="btn btn-small btn-success" title="Редактировать"  onclick="redirect2EditRecord(' + data[i].id + '); return false;"><i class="icon-edit"> </i></button><button type="button" class="btn btn-small btn-danger" title="Удалить" onclick="delRecord(' + data[i].id + '); return false;"><i class="icon-trash"> </i></button></div>']);
                                }
            ...
            function setTask(id_call, src, dst, call_type) {
                ...
                function (data) {
                    ...
                    $('#formTask input#selectContact').val(data);
...
// /project_dir/index.html
            $(document).ready(function () {
                ...
                var pathArray = $(location).attr('hash');
                if (pathArray[0] === '#') {
                    $('.nav-tabs a[href='+pathArray+']').tab('show') ;
                } 

                $("#phone_number").mask("9-999-999-9999", {autoclear: false});
                $("#mobile_number").mask("9-999-999-9999", {autoclear: false});
                $("#private_phone_number").mask("9-999-999-9999", {autoclear: false});
                $('#phone_number').bind("change paste keyup", function () {
                    //alert($(this).val());
                });
                ...
                $('#contacts_list #allContactsTable').dataTable({
                    "sPaginationType": "bootstrap",
                    "oLanguage": {
                        "sUrl": "/assets/js/dataTables.russian.txt"
                    },
                ...
                $('#organizations_list #allOrganizationsTable').dataTable({
                    "sPaginationType": "bootstrap",
                    "oLanguage": {
                        "sUrl": "/assets/js/dataTables.russian.txt"
                    },
...
//                });
            ...
            function deleteContactDetails(id) {
                ...
                bootbox.dialog("Удалить контакт?", [{
                        "label": "Да",
                        "class": "btn-default",
                        "callback": function () {
                            $.post('<?php echo site_url('/addressbook/deleteContactDetails'); ?>', {'id': id},
                            function (data) {
//                            var message = "Контакт удалён";
//                            msg_system(message, 'success');
                                setTimeout(function () {
                                    window.location.href = "/addressbook#contacts_list";
                                }, 2000);
            ...
            function editContactDetails(id) {
                window.location.href = "/addressbook/editContactDetails/" + id;
            }
        ...
        <style>
            ...
            #allOrganizationsTable, #allContactsTable{
                font-size:12px;
            }
